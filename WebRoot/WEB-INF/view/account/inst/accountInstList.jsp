...
<div class="widget box" id="widgetBox">
	...
	<div class="widget-content  no-padding">
		...
		<div class="dataTables_header clearfix">
			...
			<form class="searchForm" action="accountInst/search" method="post"
				...
				<table class="searchTable">
					...
					<tr>
						...
						<td nowrap="nowrap">冻结金额总计：<span class="red">￥${sumFrozen }
								元</span></td>
					</tr>
				</table>
			</form>
		...
		<table
			...
			<tbody>
				<c:forEach items="${accountInstList }" var="item" varStatus="status">
					<tr>
						<td>${status.index+(page.index-1)*page.size+1}</td>
						<td>${item.AccountNo}</td>
						<td><c:forEach items="${InstOnlineList }" var="instOnline">
								<c:if test="${fn:contains(item.AccountNo, instOnline.InstNo) and fn:contains(item.AccountNo, 'AI_')}">
									${instOnline.InstNo}
								</c:if>
							</c:forEach></td>
						<td><c:forEach items="${InstOnlineList }" var="instOnline">
				...
				<c:if test="${empty accountInstList }">
					<tr class="odd gradeX">
