...
import cn.online.nocard.web.model.base.BaseOrder;
...
//		}
	 ...
	 * @return
	 */
	public Page<Order> searchDaifu(ESSearchCondition sc) {
		int pageNumber = sc.page.getIndex();
		int pageSize = sc.page.getSize();
		String startCreateTime = sc.getString("startCreateTime");
		String endCreateTime = sc.getString("endCreateTime");
	 ...
	 * @return
	 */
	public Page<Order> searchDaifuRen(ESSearchCondition sc) {
		int pageNumber = sc.page.getIndex();
		int pageSize = sc.page.getSize();
		String startCreateTime = sc.getString("startCreateTime");
		String endCreateTime = sc.getString("endCreateTime");

		ESSQLHelper sql = new ESSQLHelper(" from t_order  WHERE 1=1 ");
		String orderStatus = sc.getString("OrderStatus");
		if(ESBoolean.isEmpty(orderStatus)){
		}else if("01".equals(orderStatus)){
			sql.notEquals("OrderStatus", "00");
		...
		sql.equals("ChName", "人工转账平台");
		
		sql.equals("ChId", "C124");
	...
	public Order searchByOrderNo(String orderNo) {
		return super.findFirst("SELECT * FROM t_order WHERE OrderNo = ? ", orderNo);
	}
